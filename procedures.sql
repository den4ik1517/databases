...
BEGIN
    ...
    IF operator = 'MAX' THEN
        SELECT MAX(Cost) INTO result FROM iot_db.orders;
    ELSEIF operator = 'MIN' THEN
        SELECT MIN(Cost) INTO result FROM iot_db.orders;
    ELSEIF operator = 'AVG' THEN
        SELECT AVG(Cost) INTO result FROM iot_db.orders;
    ELSEIF operator = 'SUM' THEN
        SELECT SUM(Cost) INTO result FROM iot_db.orders;
    END IF;
END//
DELIMITER ;
...
BEGIN
    ...
    DECLARE cur_place CURSOR FOR SELECT * FROM iot_db.places;
